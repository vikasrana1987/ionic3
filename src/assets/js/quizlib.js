...
Quiz.prototype.checkAnswers = function(flagUnanswered) {
	...
	for (var i=0; i < this.questions.length; i++) {
		...
		// Get answers
		var answerInputs = question.getElementsByClassName(Quiz.Classes.QUESTION_ANSWERS)[0];
		var ion_item = answerInputs.getElementsByTagName('ion-item');
		for (var k=0; k < ion_item.length; k++) {
			...
			if(this.hasClass(item,'item-input'))
			{
				...
				if (input[0].value !== '') {
					userAnswer.push(input[0].value);
				}
			}
			
			if(this.hasClass(item,'item-radio'))
			{
				if(this.hasClass(item,'item-radio-checked'))
				{
					radio = item.getElementsByTagName('ion-radio');
					userAnswer.push(radio[0].getAttribute('value'));
				}
			}
			if(this.hasClass(item,'item-checkbox'))
			{
				
				if(this.hasClass(item,'item-checkbox-checked'))
				{
					checkbox = item.getElementsByTagName('ion-checkbox');
					userAnswer.push(checkbox[0].getAttribute('value'));
				}
			} 
		}
...
Quiz.prototype.hasClass = function(element, cls) {
